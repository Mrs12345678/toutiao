...
<script>
import { sendSms,reg } from "@/api/user";
...
// 点击事件发送
    ...
    getuser() {
      ...
      reg({  username,password, vercode,}).then((res) => {
        console.log(res);
        if (res.code == 0) {
          this.$toast("注册成功");
          localStorage.setItem("tt-token", token);
          // localStorage.setItem("tt-username", username);
          // localStorage.setItem('tt-uid',uid)
          setTimeout((v) => {
            this.$router.back();
          });
        } else {
    ...
    getsendSms() {
      ...
      sendSms({ mobile: username, type: "register",}) .then((res) => {
          console.log(res);
          // 发送成功
          if (res.code == 0) {
            this.countdown();
